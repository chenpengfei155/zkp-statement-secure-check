...
ZKAP_TO_CIRVERIFY = {v: k for k, v in CIRVERIFY_TO_ZKAP.items()}

all_bug_types = list(CIRVERIFY_TO_ZKAP.keys())
    ...
    if os.path.exists(json_path):
        ...
        if groundtruth in ["safe", "unsafe"]:
            for bug in all_bug_types:
                ...
                #     continue
                in_zkap = CIRVERIFY_TO_ZKAP[bug] in expected_detetector
                in_cirverify = bug in cirverify_bugs

                if in_zkap and in_cirverify:
                    vul_stats[bug]['TP'] += 1
                elif not in_zkap and in_cirverify:
                    vul_stats[bug]['FP'] += 1
                    print("+++ false positive =", json_path, bug)
                elif in_zkap and not in_cirverify:
                    vul_stats[bug]['FN'] += 1
                    print("--- false negative =", json_path, bug)
                elif not in_zkap and not in_cirverify:
                    vul_stats[bug]['TN'] += 1

            zksolid_values = col[13:22]
            for i, bug_short in enumerate(ZKSOLID_BUGS):
                cir_bug = ZKSOLID_TO_CIRVERIFY[bug_short]
                zkap_bug_name = CIRVERIFY_TO_ZKAP[cir_bug]
...
cirverify_stats = evaluate(TP, TN, FP, FN)
...
for tool, stats in zip(["CirVerify", "ZKAP", "CIRCOMSPECT"],
                       [cirverify_stats, zkap_stats, circomspect_stats]):
    print(f"\n=== {tool} ===")
