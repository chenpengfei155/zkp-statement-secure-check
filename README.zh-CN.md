...
| Analyze | 检查当前标签页：Circom 运行源码检测，R1CS 运行有限的约束检查 |
...
4. 默认选择 **Signed / 简洁系数**，例如把 `p−1` 显示为 `−1`；选择
   **Original / 原始系数** 可以查看原来的非负大整数。两种显示都按文件中的模数计算，
   数学含义相同，不会修改上传文件。
5. 点击 **Analyze**，在工作区下方查看报告：包含发现的问题、变量编号、约束编号，
   以及原有 9 类检测项的覆盖范围。可以点击 **Stop** 取消；结果保存在对应文件标签页中。

项目里的 `demo1.r1cs` 应显示：6 条约束、7 个变量（含常量）、2 个公开输出、
0 个公开输入和 4 个私有输入。切换文件标签页时会保留源码编辑内容，以及 R1CS 的
当前页和系数显示模式；再次上传同名文件，会替换对应标签页的内容。

R1CS 目前提供**有限的约束检查**：公开输出或保留的变量未参与有效约束、恒成立的
`0 = 0` 约束、无法成立的常量约束，以及有限规模的线性推导可以确认的约束矛盾。
检查前按文件模数合并系数，因此 `x × 0 = 0` 不会被误认为限制了 `x`。
未使用的非输出变量、恒成立的约束仅列为“提示”，不直接判定为安全漏洞。

原有 9 类检测中，“输出未受约束”和“未使用的信号”仅能**部分检查**。
其余 7 类（组件输入未受约束、计算与约束不一致、未使用的组件输出、类型不匹配、
赋值误用、除零风险、非确定性数据流）需要源码信息，报告明确显示**无法检查**，
不会显示成“通过”。**未发现问题不代表电路安全**：本次不验证输出唯一性、全部非线性
约束的可满足性或业务逻辑。例如 `x² = 1` 中的 `x` 虽然出现在约束里，仍可能有多个取值。

检查使用 20 秒的协作式时间预算，并限制多项式展开和线性消元的计算量；达到限制时会
标明“部分检查未完成”。最多展示前 200 条发现，但统计包含其余记录。同一时间只运行
一个 R1CS 分析。无需额外安装求解器或 Node.js。
此功能不还原 Circom 源码、不接收 `.sym`、不生成证明。目前不支持带自定义门扩展的文件。
文件夹选择和命令行仍然只处理 Circom 源码。
